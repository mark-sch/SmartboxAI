...
def tool_result_to_message(tool_result: ToolResult) -> Message:
    ...
    if tool_result.return_value.is_error:
        ...
        elif not any(isinstance(part, TextPart) and part.text.strip() for part in content):
            # Ensure at least one non-empty TextPart exists so the LLM API won't reject
